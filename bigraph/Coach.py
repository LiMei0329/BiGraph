...
class Coach:
    def __init__(self, trainset, devset, testset, model, opt, sched, args):
        ...
        if args.local_global_cat == 0:
            
            if args.dataset == "mosei":
                train_log_filename = args.dataset+"_"+args.emotion+"_n"+str(args.n_neighbors)+"_w"+str(args.wp)+"_"+str(args.learning_rate)+"_"+str(args.drop_rate)+"_head"+str(args.gnn_nheads)+".txt"
                result_dir = '/Bigraph/'
            else:
                train_log_filename = args.dataset+"_n"+str(args.n_neighbors)+"_w"+str(args.wp)+"_"+str(args.learning_rate)+"_"+str(args.drop_rate)+"_head"+str(args.gnn_nheads)+".txt"
                result_dir = '/Bigraph/'
        elif args.local_global_cat == 1:
            
            if args.dataset == "mosei":
                train_log_filename = args.dataset+"_"+args.emotion+"_w"+str(args.wp)+"_"+str(args.learning_rate)+"_"+str(args.drop_rate)+".txt"
                result_dir = '/LCE/'
            else:
                train_log_filename = args.dataset+"_w"+str(args.wp)+"_"+str(args.learning_rate)+"_"+str(args.drop_rate)+".txt"
                result_dir = '/LCE/'
        elif args.local_global_cat == 2:
            
            if args.dataset == "mosei":
                train_log_filename = args.dataset+"_"+args.emotion+"_n"+str(args.n_neighbors)+"_"+str(args.learning_rate)+"_"+str(args.drop_rate)+".txt"
                result_dir = '/GSC/'
            else:
                train_log_filename = args.dataset+"_n"+str(args.n_neighbors)+"_"+str(args.learning_rate)+"_"+str(args.drop_rate)+".txt"
                result_dir = '/GSC/'
        self.train_log_filepath = os.path.join(result_dir, train_log_filename)
    ...
    def train(self):
        ...
        # Early stopping.
        best_test_f1, best_epoch, best_state = (
            self.best_test_f1,
            self.best_epoch,
            self.best_state,
        )
        ...
        if self.args.tuning:
            ...
            return best_test_f1, best_epoch, best_state, train_losses, dev_f1s, test_f1s

        # The best
        train_log_txt_formatter = "Best in epoch {best_epoch}:\n [Dev set] f1 {dev_f1}\n [Test set] f1  {test_f1}\n {towrite}"
        ...
        log.info("[Dev set] [f1 {:.4f}]".format(dev_f1))
        test_f1, _, towrite1 = self.evaluate(test=True)
        log.info("[Test set] f1 {}".format(test_f1))
        to_write = train_log_txt_formatter.format(best_epoch=best_epoch,
                                                  dev_f1 = dev_f1,
                                                  test_f1=test_f1,
                                                  towrite=towrite1)
    ...
    def evaluate(self, test=False):
        ...
        with torch.no_grad():
            ...
            if test:
                x = metrics.classification_report(
