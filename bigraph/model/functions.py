...
def global_graphify(data, device, edge_type_to_idx, n_neighbors, node_features):
    features = node_features
    lengths = data["text_len_tensor"]
    speaker = data["speaker_tensor"]
    n = n_neighbors
    speaker_sum = 0
    global_speaker_tensor = []
    global_node_features = []
    global_edge_type = []
    batch_size = features.size(0)
    for j in range(batch_size):
        cur_len = lengths[j].item()
        global_node_features.append(features[j, :cur_len, :])
        global_speaker_tensor.append(speaker[j, :cur_len])
    global_node_features = torch.cat(global_node_features, dim=0).to(device) 
    global_speaker_tensor = torch.cat(global_speaker_tensor, dim=0).to(device)

    edges = kneighbors_graph(global_node_features.cpu().detach().numpy(), n_neighbors=n, include_self=True) 

    edges = edges.tocoo() 
    row = edges.row
    col = edges.col
 
    global_edges = torch.tensor(np.column_stack((row, col)))

    for item in global_edges:
        speaker1 = global_speaker_tensor[item[0]].item()
        speaker2 = global_speaker_tensor[item[1]].item()
        if item[0] < item[1]: 
            c = "0"
        else: 
            c = "1"
        global_edge_type.append(edge_type_to_idx[str(speaker1) + str(speaker2) + c])
    global_edges = global_edges.t().contiguous().long().to(device)

    global_edge_type = torch.tensor(global_edge_type).long().to(device)
    return global_node_features, global_edges, global_edge_type
    
    
def local_graphify(features, lengths, speaker_tensor, wp, wf, edge_type_to_idx, device):
    node_features, edge_index, edge_type = [], [], []
    batch_size = features.size(0) #[batch_size, mx_len, D_g] 
    length_sum = 0
    edge_ind = []
    edge_index_lengths = []
    ...
    edge_index_lengths = torch.tensor(edge_index_lengths).long().to(device)  # [Batch_size]
