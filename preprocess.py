...
def get_iemocap():
    ...
    for vid in tqdm(train_vids, desc="train"):#desc 传入进度条的前缀
        train.append(
            bigraph.Sample(
                vid,
                video_speakers[vid],
...
def main(args):
    if args.dataset == "iemocap":
        ...
        bigraph.utils.save_pkl(data, "./data/iemocap/data_iemocap.pkl")
    if args.dataset == "iemocap_4" and args.split_utterances == -1:
        train, dev, test = get_iemocap()
        data = {"train": train, "dev": dev, "test": test}
        bigraph.utils.save_pkl(data, "./data/iemocap_4/data_iemocap_4.pkl")
    if args.dataset == "iemocap_4" and args.split_utterances != -1:
        train, dev, test = get_iemocap_split(args.split_utterances)
        data = {"train": train, "dev": dev, "test": test}
        bigraph.utils.save_pkl(
            data,
    ...
    if args.dataset == "mosei":
        ...
        data = {"train": train, "dev": dev, "test": test}
        bigraph.utils.save_pkl(data, "./data/mosei/data_mosei.pkl")

    if args.dataset == "mosei_tbje_2class":
        train, dev, test = get_mosei_from_tbje(args)
        data = {"train": train, "dev": dev, "test": test}
        bigraph.utils.save_pkl(data, "./data/mosei/data_mosei_2class.pkl")

    if args.dataset == "mosei_tbje_7class":
        train, dev, test = get_mosei_from_tbje(args)
        data = {"train": train, "dev": dev, "test": test}
        bigraph.utils.save_pkl(data, "./data/mosei/data_mosei_7class.pkl")

    if args.dataset == "mosei_emotion":
        train, dev, test = get_mosei_from_tbje_emotion(args)
        data = {"train": train, "dev": dev, "test": test}
        bigraph.utils.save_pkl(data, "./data/mosei/data_mosei_" + args.emotion + ".pkl")
