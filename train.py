...
def func(experiment, trainset, devset, testset, model, opt, sched, args):
    ...
    coach = bigraph.Coach(trainset, devset, testset, model, opt, sched, args)
...
def main(args):
    bigraph.utils.set_seed(args.seed)
    ...
    data = bigraph.utils.load_pkl(args.data)
    log.info("Loaded data.")

    trainset = bigraph.Dataset(data["train"], args)
    devset = bigraph.Dataset(data["dev"], args)
    testset = bigraph.Dataset(data["test"], args)
    ...
    model = bigraph.BiGraph(args).to(args.device)
    opt = bigraph.Optim(args.learning_rate, args.max_grad_value, args.weight_decay)
...
if __name__ == "__main__":
    ...
    """ Dataset specific info (effects)
            -> tag_size in BiGraph.py
            -> n_speaker in BiGraph.py
    ...
    parser.add_argument("--device", type=str, default="cuda:0", help="Computing device.")#"cuda:1"
    ...
    parser.add_argument(
        ...
        type=int,
        default=7,
        help="n_neighbors in KNeighborsClassifier.",
    )

    parser.add_argument("--alpha", type=float, default=0.5, help="alpha*graph_global+(1-alpha)*graph_local.")
    ...
    # emotion
    parser.add_argument(
        "--emotion", type=str, default=None, help="emotion class for mosei, such as: 7class 2class multilabel"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="0:use local and gobal graph, 1:only use local graph, 2:only use global graph.",
    )
